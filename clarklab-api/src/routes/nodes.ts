...
function buildDevRegisterCommand(token: string): string {
  return `./clarklab-agent register --token ${token} --server ${config.serverUrl} --config ${DEV_AGENT_CONFIG}`
}

function buildDevRunCommand(): string {
...
function buildRegistrationPayload(
  ...
  return {
    ...
    devRegisterCommand: appendDataRootFlag(buildDevRegisterCommand(token), dataRoot),
...
function rowToNode(row: Record<string, unknown>) {
  ...
  return {
    ...
    accessMode: ((row.access_mode as string) === 'projects' ? 'projects' : 'all') as NodeAccessMode,
...
nodeRoutes.patch('/:nodeId', async (c) => {
  ...
  if (body.accessMode !== undefined) {
    const accessMode: NodeAccessMode = body.accessMode === 'projects' ? 'projects' : 'all'
    ...
    await setNodeAllowedProjects(nodeId, accessMode, projectIds)
  }
...
agentRoutes.post('/tasks/:taskId/complete', async (c) => {
  ...
  const taskResult = await pool.query(
    `SELECT t.id, t.action, se.node_id, se.environment, se.id AS service_environment_id, se.port, s.id AS service_id
     FROM deploy_tasks t
  ...
  const serviceUrl = formatServiceAccessUrl(nodeIp, port ?? 0)
    ...
    if (previousIp !== localIp) {
      ...
      await pool.query(
        `UPDATE service_environments
         ...
         WHERE node_id = $1 AND port IS NOT NULL AND port > 0`,
        [nodeId, localIp],
      )
    }
    await pool.query(
