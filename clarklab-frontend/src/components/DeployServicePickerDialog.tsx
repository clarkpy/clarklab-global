        ...
        <DialogContent className="theme-surface-inner max-h-[90vh] overflow-y-auto border text-foreground shadow-[0_30px_120px_rgba(0,0,0,0.65)] light:shadow-[0_20px_60px_rgba(124,58,237,0.1)] sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription className="theme-muted text-sm leading-6">
              Choose which service to deploy to its assigned node.
            </DialogDescription>
          </DialogHeader>
          ...
          {loading ? (
            ...
            <div className="space-y-4 py-2">
              <input
                ...
                className="theme-glass theme-heading w-full rounded-2xl border px-4 py-2.5 text-sm"
              />
          ...
          {services.length > 0 ? (
            <DialogFooter className="gap-2 sm:gap-0">
              <button
                type="button"
                onClick={onClose}
                className="theme-btn-secondary rounded-full px-5 py-2 text-sm font-semibold transition"
              >
                Cancel
              </button>
              <button
                ...
                disabled={!selected}
                className="rounded-full border border-violet-400 bg-gradient-to-r from-purple-600 to-purple-500 px-5 py-2 text-sm font-semibold text-white transition hover:border-violet-300 disabled:cursor-not-allowed disabled:opacity-50"
              >
                Continue
              </button>
            </DialogFooter>
