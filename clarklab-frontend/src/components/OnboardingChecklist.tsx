...
export function OnboardingChecklist({ steps, onDismiss }: OnboardingChecklistProps) {
  ...
  const firstIncompleteRequiredId = steps.find((step) => !step.optional && !step.complete)?.id
        ...
        {steps.map((step, index) => (
          <li key={step.id} className="flex items-start gap-4">
            ...
            <div className="min-w-0 flex-1 pb-2">
              <p className={`text-sm font-semibold ${step.complete ? 'theme-accent-emerald' : step.id === firstIncompleteRequiredId ? 'theme-heading font-black' : 'theme-heading'}`}>
                {step.label}
                {step.optional ? (
