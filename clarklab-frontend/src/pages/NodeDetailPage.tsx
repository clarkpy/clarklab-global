...
import { HEARTBEAT_INTERVAL_MIN, HEARTBEAT_INTERVAL_MAX, LATEST_AGENT_VERSION } from '@/lib/config'
...
function SettingsField({
  ...
  return (
    <label className="block">
      <span className="theme-muted text-xs uppercase tracking-[0.25em]">{label}</span>
      {hint && <p className="theme-subheading mt-2 text-sm leading-6">{hint}</p>}
...
export default function NodeDetailPage() {
  ...
  const [accessMode, setAccessMode] = useState<'all' | 'projects'>('all')
  ...
  const [latestAgentVersion, setLatestAgentVersion] = useState(LATEST_AGENT_VERSION)
  ...
  useEffect(() => {
    fetchClarklabConfig()
      .then((config) => setLatestAgentVersion(config.latestAgentVersion))
      .catch((err: unknown) => {
        toast.failed(getFetchErrorMessage(err))
      })
  }, [])
  ...
  useEffect(() => {
    ...
    if (setupLocked) {
      ...
      setActiveTab((prev) => (prev === 'overview' ? 'setup' : prev))
    }
  }, [setupStatus?.complete, nodeId, registration, setupLocked])
  ...
  if (loading) {
    return (
      <div className="relative mx-auto w-full max-w-6xl px-6 pb-10 md:px-8 lg:px-12">
        <p className="theme-muted text-sm">Loading node…</p>
      </div>
    )
  }
    ...
    <div className="relative mx-auto w-full max-w-6xl px-6 pb-10 md:px-8 lg:px-12">
      <Reveal delay={0}>
        <nav className="theme-muted flex flex-wrap items-center gap-1.5 text-sm font-semibold">
          ...
          <span className="theme-heading">{node.name}</span>
        </nav>
        ...
        <div className="mt-6 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            {node.description && (
              <p className="theme-subheading mt-2 max-w-2xl text-sm leading-6">{node.description}</p>
            )}
            ...
            {setupIncomplete && (
              <p className="theme-accent-amber mt-2 text-sm font-semibold">{node.lastSeenAt}</p>
            )}
          </div>
          ...
          <div className="flex w-full flex-col gap-3 lg:w-auto lg:items-end">
            {(node.status === 'offline' || node.status === 'degraded') && (
              <button
                ...
                className="theme-glass inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold transition hover:border-violet-400/40 hover:text-violet-300 light:hover:text-violet-700 disabled:cursor-not-allowed disabled:opacity-50"
              >
      ...
      {(node.status === 'pending' || setupIncomplete) && showSetupTab ? (
        <div className="mt-6 rounded-2xl border border-amber-400/30 bg-amber-500/10 px-5 py-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="theme-accent-amber text-sm font-semibold">Finish node setup</p>
              <p className="theme-muted mt-1 text-xs leading-6">
                Run the install or register commands, then start the agent on your host.
              </p>
            </div>
            <button
              type="button"
              onClick={() => setActiveTab('setup')}
              className="shrink-0 rounded-full border border-amber-400/40 bg-amber-500/20 px-4 py-2 text-sm font-semibold text-amber-100 transition hover:border-amber-300/50 light:text-amber-900"
            >
              View setup steps
            </button>
          </div>
      ...
      <Reveal delay={80}>
        ...
        >
          ...
          <TabsContent value="overview" className="mt-6 space-y-6">
            {canReconnect && !showSetupTab && (
              <div className="theme-glass rounded-2xl border border-rose-400/25 px-5 py-5">
                ...
                <button
                  ...
                  onClick={handleReconnect}
                  disabled={reconnecting}
                  className="mt-4 inline-flex items-center gap-2 rounded-full border border-violet-400 bg-gradient-to-r from-purple-600 to-purple-500 px-5 py-2.5 text-sm font-semibold text-white transition hover:border-violet-300 disabled:cursor-not-allowed disabled:opacity-50"
                >
                  <RefreshCw className={`h-4 w-4 ${reconnecting ? 'animate-spin' : ''}`} aria-hidden="true" />
            ...
            <NodeAgentHealth node={node} latestAgentVersion={latestAgentVersion} />
          ...
          <TabsContent value="settings" className="mt-6 space-y-6">
                ...
                {dataRootInput.trim() !== savedDataRoot && (
                  <label className="flex items-start gap-3 text-sm">
                    <input
                      ...
                      className="mt-1"
                    />
                    <span className="theme-muted leading-6">
                      Migrate existing service data to the new directory. Managed containers are stopped
                      during migration and are not restarted automatically.
                    </span>
                  </label>
                )}
            ...
            <Card className="p-6">
              ...
              <p className="theme-subheading mt-2 text-sm leading-6">
                Control which projects can deploy services to this node.
              </p>
                      ...
                      {projectOptions
                        ...
                        .map((project) => (
                          <label key={project.id} className="flex items-center gap-2 text-sm">
                            ...
                            />
                            <span className="theme-heading font-semibold">{project.name}</span>
                          </label>
                        ))}
                    </div>
                  </div>
